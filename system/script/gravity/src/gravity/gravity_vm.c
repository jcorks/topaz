            ...
            CASE_CODE(LOADAT):{
                ...
                switch(closure->f->tag) {
                    ...
                    case EXEC_TYPE_INTERNAL: {
                        ...
                        bool result = closure->f->internal(vm, &stackstart[rwin], 2, r1, closure->f->internalData);
            ...
            CASE_CODE(CALL): {
                ...
                switch(closure->f->tag) {
                    ...
                    case EXEC_TYPE_INTERNAL: {
                        ...
                        gravity_value_t r1copy = STACK_GET(r1);
                        BEGIN_TRUST_USERCODE(vm);
                        bool result = closure->f->internal(vm, &stackstart[rwin], r3, r1, closure->f->internalData);
                        END_TRUST_USERCODE(vm);
                        if (!result) {
                            if (vm->aborted) return false;
...
bool gravity_vm_runclosure (gravity_vm *vm, gravity_closure_t *closure, gravity_value_t sender, gravity_value_t params[], uint16_t nparams) {
    ...
    switch (f->tag) {
        ...
        case EXEC_TYPE_INTERNAL:
            BEGIN_TRUST_USERCODE(vm);
            result = f->internal(vm, &stackstart[rwin], nparams, GRAVITY_FIBER_REGISTER, closure->f->internalData);
            END_TRUST_USERCODE(vm);
            break;
