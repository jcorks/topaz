...
static GLTexAtlas * atlas_create(topazGL3_TexMan_t *, int index, GLuint);
...
static int atlas_request_region(GLTexAtlas * atlas, topazGL3_Texture_t * texture);

// releases the region in use by the texture
static void atlas_release_region(GLTexAtlas * atlas, topazGL3_Texture_t * texture);
...
topazGL3_TexMan_t * topaz_gl3_texman_create() {
    TOPAZ_GLES_FN_IN;
    topazGL3_TexMan_t * out = calloc(1, sizeof(topazGL3_TexMan_t));
...
void topaz_gl3_texman_destroy(topazGL3_TexMan_t * tm) {
    TOPAZ_GLES_FN_IN;
    int i;
    for(i = 0; i < 16; ++i) {
...
const GLuint * topaz_gl3_texman_gl_textures(const topazGL3_TexMan_t * a) {
    return a->ids;
}
...
void topaz_gl3_texman_print_section(
    ...
    for(y = 0; y < h; ++y) {
        printf("|");
        for(x = 0; x < w; ++x) {
            printf("%c", topaz_gl3_get_char_from_pixel(iter));
            iter+=4;
        }
        printf("|\n");
    }
...
topazGL3_Texture_t * topaz_gl3_texture_create(
    ...
    topazGL3_Texture_t * out = calloc(1, sizeof(topazGL3_Texture_t));
    out->w = w;
    out->h = h;
    out->x = -1;
    out->y = -1;
...
void topaz_gl3_texture_destroy(
    topazGL3_Texture_t * t
) {
    uint32_t i;
    uint32_t len = topaz_array_get_size(t->src->textures);
    for(i = 0; i < len; ++i) {
        if (topaz_array_at(t->src->textures, topazGL3_Texture_t *, i) == t) {
...
static GLTexAtlas * atlas_create(topazGL3_TexMan_t * tm, int index, GLuint texture) {
    TOPAZ_GLES_FN_IN;
    ...
    out->textures = topaz_array_create(sizeof(topazGL3_Texture_t *));
...
void atlas_release_region(GLTexAtlas * a, topazGL3_Texture_t * t) {
    a->needsReshuffle = 1;
}
...
void atlas_resize(GLTexAtlas * a, int newWidth, int newHeight) {
    ...
    for(i = 0; i < len; ++i) {
        topazGL3_Texture_t * texture = topaz_array_at(a->textures, topazGL3_Texture_t *, i);
    ...
    topazArray_t * changedTex = topaz_array_create(sizeof(topazGL3_Texture_t*));
...
void atlas_reset(GLTexAtlas * a) {
    ...
    topazArray_t * changedTex = topaz_array_create(sizeof(topazGL3_Texture_t*));
    for(i = 0; i < len; ++i) {
        topazGL3_Texture_t * texture = topaz_array_at(cl, topazGL3_Texture_t *, i);
