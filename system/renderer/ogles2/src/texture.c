...
static void atlas_read(topazES2_Atlas_t *, int x, int y, int w, int h, uint8_t * target);
...
static int atlas_request_region(topazES2_Atlas_t * atlas, topazES2_Texture_t * texture);

// releases the region in use by the texture
static void atlas_release_region(topazES2_Atlas_t * atlas, topazES2_Texture_t * texture);

// Writes to the texel space
static void atlas_write(topazES2_Atlas_t *, int x, int y, int w, int h, const uint8_t * target);
...
static void atlas_reset(topazES2_Atlas_t *);

// resizes the atlas. The unfortunately requires re-downloading all textures 
// and re-uploading them.
static void atlas_resize(topazES2_Atlas_t *, int w, int h);
...
void topaz_es2_atlas_destroy(topazES2_Atlas_t * a) {
    TOPAZ_GLES_FN_IN;
...
void atlas_read(topazES2_Atlas_t * a, int x, int y, int w, int h, uint8_t * target) {
    TOPAZ_GLES_FN_IN;
...
static int atlas_request_region(topazES2_Atlas_t * a, topazES2_Texture_t * t) {
    ...
    // then resize width as far as possible
    if (a->width < a->maxLength) { 

        a->width += ATLAS_RESIZE_AMT;
        if (a->width > a->maxLength)
            a->width = a->maxLength;
    ...
    // then try to move to "next row"
    if (a->iterY + t->h + ATLAS_SPACE_BORDER < a->height) {
        a->iterY += a->iterHeight + ATLAS_SPACE_BORDER;
        a->iterX = 0;
        return atlas_request_region(a, t);
    }


    // finally resize height
...
void atlas_release_region(topazES2_Atlas_t * a, topazES2_Texture_t * t) {
    a->needsReshuffle = 1;
}

void atlas_resize(topazES2_Atlas_t * a, int newWidth, int newHeight) {
    TOPAZ_GLES_FN_IN;
...
void atlas_reset(topazES2_Atlas_t * a) {
    if (!a->needsReshuffle) return;

    uint32_t i, n;
    uint32_t len = topaz_array_get_size(a->textures);
    uint32_t cbCount = topaz_array_get_size(a->cbs);
    // have to copy all the texture data to CPU memory
    for(i = 0; i < len; ++i) {
        topazES2_Texture_t * texture = topaz_array_at(a->textures, topazES2_Texture_t *, i);
        uint8_t * dataBuffer = malloc(texture->w * texture->h * 4);
    ...
    for(i = 0; i < len; ++i) {
        ...
        for(n = 0; n < cbCount; ++n) {
            ResetCB * cb = &topaz_array_at(a->cbs, ResetCB, n);
...
static void atlas_write(topazES2_Atlas_t * a, int x, int y, int w, int h, const uint8_t * target) {
    TOPAZ_GLES_FN_IN;
    glBindTexture(GL_TEXTURE_2D, a->object); TOPAZ_GLES_CALL_CHECK;
