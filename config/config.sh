...
for D in *; do
    if [ -d "${D}" ]; then
        ...
        if [ $result -eq "0" ]; then 
            ...
            configsource=$(echo -e "    ${configsource}\ntopaz_system_add_${backendType}_handler(\"${backendName}\", topaz_system_${backendType}_${backendName}__backend, (void (*)(void *)) topaz_system_${backendType}_${backendName}__api);")
...
echo -e "void topaz_system_config_auto() {\n" >> config.c
