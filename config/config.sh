...
for D in *; do
    if [ -d "${D}" ]; then
        ...
        if [ $result -eq "0" ]; then 
            ...
            configsource=$(echo -e "    ${configsource}\ntopaz_system_config_add_handler(TOPAZ_STR_CAST(\"${backendType}\"), TOPAZ_STR_CAST(\"${backendName}\"), topaz_system_${backendType}_${backendName}__backend, (void (*)(void *)) topaz_system_${backendType}_${backendName}__api);")
...
echo -e "#include <topaz/system.h>" > config.c
echo "$configheader" >> config.c
echo -e "void topaz_system_configure_base() {\n" >> config.c
